...
export default async function SearchPage({
  ...
  if (!q) {
    return (
      <main className="min-h-screen bg-white px-5 py-20 text-center text-black">
        ...
        <p className="mt-4 text-gray-500">Please enter a search term in the header.</p>
      </main>
    );
  }
  ...
  let totalProducts = 0;
  try {
    const data = await getProducts(`?search=${encodeURIComponent(q)}&limit=75&page=${page}`);
  ...
  return (
    <main className="min-h-screen bg-white px-5 py-14 sm:px-8 lg:px-12">
      <div className="mx-auto max-w-7xl">
        <div className="mb-10">
          <h1 className="font-serif text-3xl font-bold text-black sm:text-4xl">
        ...
        {products.length === 0 ? (
          <div className="py-20 text-center">
            <p className="text-gray-500">No jerseys match your search.</p>
            <Link
              href="/shop"
